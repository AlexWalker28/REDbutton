...
Generated jwts will include an `iat` (issued at) claim by default unless `noTimestamp` is specified. If `iat` is inserted in the payload, it will be used instead of the real timeInMillis for calculating other things like `exp` given a timespan in `options.expiresIn`.
